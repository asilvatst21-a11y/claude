...
export function X1WinnerCard({ winnerName, loserName, winnerPoints, loserPoints, isCurrentUserWinner }: Props) {
  ...
  const getShareText = () =>
    buildShareText(isCurrentUserWinner, winnerName, loserName, winnerPoints, loserPoints, seed, getUrl());
    ...
    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl overflow-hidden mb-6">
      ...
      <div className="flex items-stretch divide-x divide-zinc-700">
        <div className="flex-1 flex flex-col items-center justify-center py-5 px-4 gap-1">
          ...
          <p className="text-sm font-semibold text-white text-center">{winnerName}</p>
        ...
        <div className="flex-1 flex flex-col items-center justify-center py-5 px-4 gap-1">
          <span className="text-3xl">💀</span>
          <p className="text-sm font-semibold text-zinc-400 text-center">{loserName}</p>
      ...
      <div className="px-5 py-4 border-t border-zinc-700 flex flex-col gap-2">
        <p className="text-xs text-zinc-500 text-center mb-1">
          {isCurrentUserWinner
            ? "Compartilha essa vitória e zoa o perdedor 😂"
            : "Conta a derrota (ou pede revanche) 😅"}
        </p>
        ...
        {/* Share nativo (aparece só no mobile) */}
        {canNativeShare && (
          <button
